...
import { ArrowDownTrayIcon, PlusIcon, ViewColumnsIcon, DocumentIcon, CodeBracketIcon, XMarkIcon, ClipboardDocumentIcon, CommandLineIcon, SparklesIcon, AdjustmentsHorizontalIcon, CheckIcon, GlobeAltIcon, CloudArrowUpIcon, MusicalNoteIcon, PhotoIcon } from '@heroicons/react/24/outline';
...
interface LivePreviewProps {
  ...
  isFocused: boolean;
  onReset: () => void;
  onRefine: (prompt: string, files: File[]) => void;
}
...
export const LivePreview: React.FC<LivePreviewProps> = ({ creation, isLoading, isFocused, onReset, onRefine }) => {
    ...
    const [ghRepo, setGhRepo] = useState(localStorage.getItem('gh_repo') || 'my-gemini-site');
...
<body>
    ...
    const handleGitHubPublish = async () => {
        if (!creation?.html || !ghToken || !ghRepo) return;
        
        setPublishStatus('loading');
        setPublishMessage('Connecting to GitHub...');
        
        // Save to LS for convenience
        ...
        try {
            ...
            if (!userReq.ok) throw new Error('Invalid Token or Network Error');
            const userData = await userReq.json();
            const username = userData.login;

            // 2. Check if Repo Exists
            setPublishMessage('Checking repository...');
            let repoReq = await fetch(`https://api.github.com/repos/${username}/${ghRepo}`, {
                headers: { Authorization: `token ${ghToken}` }
            });

            // 3. Create Repo if 404
            if (repoReq.status === 404) {
                 setPublishMessage('Creating new repository...');
                 repoReq = await fetch('https://api.github.com/user/repos', {
                    method: 'POST',
                    headers: { 
                    ...
                    body: JSON.stringify({
                        name: ghRepo,
                        description: 'Generated by Gemini Smart Browser',
                        auto_init: true,
                        private: false // Free pages usually need public
                    })
                });
                if (!repoReq.ok) throw new Error('Failed to create repository');
                // Wait a moment for creation to propagate
                await new Promise(r => setTimeout(r, 2000));
            }

            // 4. Get SHA of index.html if it exists (for update)
            setPublishMessage('Preparing files...');
            const fileUrl = `https://api.github.com/repos/${username}/${ghRepo}/contents/index.html`;
            ...
            setPublishMessage('Pushing code to GitHub...');
            // Base64 encode properly handles UTF-8
            const contentEncoded = btoa(unescape(encodeURIComponent(creation.html)));
            
            const putReq = await fetch(fileUrl, {
                method: 'PUT',
                headers: { 
                    Authorization: `token ${ghToken}`,
                    'Content-Type': 'application/json'
            ...
            if (!putReq.ok) throw new Error('Failed to push code');
            
            setPublishStatus('success');
            setPublishMessage(`Published! Check https://${username}.github.io/${ghRepo}/`);
            
            // Optional: Enable Pages (requires more complex API calls usually, or user manual setup)
            // But we can link them to the repo
            
        } catch (e: any) {
            console.error(e);
            setPublishStatus('error');
            setPublishMessage(e.message || "Failed to publish");
        }
    };
    ...
    const handleRefineSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (refinePrompt.trim() || refineFiles.length > 0) {
            onRefine(refinePrompt, refineFiles);
            setRefinePrompt("");
            setRefineFiles([]);
        }
    };
    ...
    const removeRefineFile = (index: number) => {
                ...
                <div className="w-80 border-l border-zinc-800 bg-[#121214] flex flex-col shrink-0 animate-in slide-in-from-right duration-300">
                     ...
                     <div className="p-4 space-y-3">
                         ...
                         <button 
                            ...
                            className="w-full flex items-center justify-between px-3 py-2 rounded text-sm transition-colors group bg-zinc-800 hover:bg-zinc-700 text-zinc-200 border border-zinc-700"
                         >
                            <span className="font-mono text-xs">Publish to GitHub</span>
                            <CloudArrowUpIcon className="w-4 h-4 text-zinc-500 group-hover:text-white" />
                         </button>
                     ...
                     <div className="border-t border-zinc-800 p-4 flex-1 flex flex-col">
                        ...
                        <p className="text-xs text-zinc-500 mb-3 leading-relaxed">
                            Don't like the result? Add audio/images or describe changes, and the engine will rewrite the code.
                        </p>
                        
                        <form onSubmit={handleRefineSubmit} className="flex-1 flex flex-col">
                            {/* File Upload for Refinement */}
                            <div className="mb-3">
                                <label className="flex items-center space-x-2 text-xs text-zinc-400 hover:text-white cursor-pointer transition-colors w-fit group border border-zinc-800 hover:border-zinc-700 bg-zinc-900/50 p-2 rounded-md">
                                    <PlusIcon className="w-3 h-3" />
                                    <span>Add Assets (Audio/Images)</span>
                                    <input 
                                        type="file" 
                                        multiple 
                                        accept="audio/*,image/*" 
                                ...
                                {refineFiles.length > 0 && (
                                    <div className="mt-2 space-y-1">
                                        {refineFiles.map((f, i) => (
                                            <div key={i} className="flex justify-between items-center bg-zinc-800/50 p-1.5 rounded border border-zinc-800">
                                               <div className="flex items-center space-x-2 overflow-hidden">
                                                   {f.type.startsWith('audio') ? (
                                                       <MusicalNoteIcon className="w-3 h-3 text-purple-400 shrink-0" />
                                                   ) : (
                                                       <PhotoIcon className="w-3 h-3 text-green-400 shrink-0" />
                                                   )}
                                                   <span className="text-[10px] text-zinc-300 truncate">{f.name}</span>
                                               </div>
                                               <button type="button" onClick={() => removeRefineFile(i)} className="text-zinc-500 hover:text-red-400">
                            ...
                            <textarea
                                ...
                                placeholder="e.g. Use the uploaded audio for background music..."
                                className="w-full flex-1 bg-zinc-900 border border-zinc-700 focus:border-blue-500 rounded p-3 text-sm text-zinc-200 placeholder-zinc-600 resize-none focus:outline-none transition-colors min-h-[100px]"
                            />
                            <button 
                                type="submit"
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 w-full max-w-sm shadow-2xl">
                  ...
                  {publishStatus === 'success' ? (
                      <div className="space-y-4">
                          <div className="p-3 bg-green-900/20 border border-green-900/50 rounded text-green-400 text-sm">
                              {publishMessage}
                          </div>
                          <div className="flex justify-end">
                              <button onClick={() => { setShowGithubModal(false); setPublishStatus('idle'); }} className="px-4 py-2 bg-zinc-800 text-white rounded text-sm hover:bg-zinc-700">Close</button>
                          </div>
                      </div>
                  ) : (
                      <div className="space-y-4">
                          <div>
                              <label className="block text-xs font-mono text-zinc-500 mb-1">GitHub Personal Access Token</label>
                              <input 
                                  type="password" 
                                  value={ghToken} 
                              ...
                              />
                              <p className="text-[10px] text-zinc-600 mt-1">Requires 'repo' scope.</p>
                          </div>
                          <div>
                              <label className="block text-xs font-mono text-zinc-500 mb-1">Repository Name</label>
                              <input 
                                  type="text" 
                                  value={ghRepo} 
                                  onChange={e => setGhRepo(e.target.value)}
                                  placeholder="gemini-site"
                                  className="w-full bg-black border border-zinc-700 rounded p-2 text-sm text-white focus:border-blue-500 outline-none"
                              />
                          </div>

                          {publishStatus === 'error' && (
                              <div className="text-red-400 text-xs p-2 bg-red-900/10 border border-red-900/50 rounded">
                                  {publishMessage}
                              </div>
                          )}
                          ...
                          <div className="flex justify-end gap-2 mt-4">
                              ...
                              <button 
                                  ...
                                  disabled={publishStatus === 'loading' || !ghToken || !ghRepo}
                                  className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded text-sm font-medium flex items-center gap-2"
                              >
                                  {publishStatus === 'loading' && <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>}
                                  Publish Update
                              </button>
                          </div>
                      </div>
