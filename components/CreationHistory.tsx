...
export interface Creation {
  ...
  name: string;
  html: string;
  originalImage?: string; // Backwards compatibility for single image
  originalImages?: string[]; // New array support
  timestamp: Date;
}
  ...
  return (
    <div className="w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="flex overflow-x-auto space-x-4 pb-2 px-2 scrollbar-hide">
        {history.map((item) => {
          const hasImage = !!item.originalImage || (item.originalImages && item.originalImages.length > 0);
